...
def test_ollama():
    ...
    try:
        ...
        if any('functiongemma' in n for n in model_names):
            console.print("[green]   ✅ FunctionGemma found[/green]")
            return True
        else:
            console.print("[yellow]   ⚠️  FunctionGemma not found[/yellow]")
            return False
...
def test_pymavlink():
    ...
    try:
        import pymavlink
        console.print(f"[green]   ✅ PyMAVLink installed[/green]")
        return True
    except:
        console.print("[red]   ❌ PyMAVLink not installed[/red]")
        return False
